...
import { getCinemaBySlug } from "../../api/CinemaAPI";
...
function ManageDetailCinema() {
  ...
  const handleTheaterClick = (id) => {
    navigate(`/admin/theater/${id}`);
  };
  useEffect(() => {
    const fetchCinemaData = async () => {
      try {
        const response = await getCinemaBySlug(slug);
...
export default ManageDetailCinema;
