...
const RowModal = ({ isOpen, onClose, row, handleRowAction }) => {
      ...
      <div className="bg-white rounded-lg p-6 w-96 transform transition-all shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold flex items-center gap-2">
            <MdEventSeat className="text-blue-500" /> Quản lý hàng {row}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-3">
          <button
            onClick={() => handleRowAction("vip", row)}
          ...
          <button
            onClick={() => handleRowAction("standard", row)}
